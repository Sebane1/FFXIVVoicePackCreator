        ...
        public void Play(bool ignoreTopMost = false) {
            ...
            if (window.AutoSyncCheckbox.Checked && index < 14) {
                if (window.SelectedVoiceDescriptor != null) {
                    decimal delay = (decimal)1000.0 * RaceVoice.TimeCodeData[window.SelectedVoiceDescriptor.RaceName + "_" 
                        + window.SelectedVoiceDescriptor.VoiceGender].TimeCodes[index];
